...
data = pd.read_csv('./perceptual_results/data_exp_183158-v8_task-bwk3.csv')
...
data = data[['Response', 'Screen', 'Participant Private ID', 'Spreadsheet: pinv']]
data = data.dropna()
...
data['audio_type'] = data['Screen']
data['audio_type'] = data['audio_type'].replace('oracle', 'mel-lj').replace('Tau', 'tau-lj').replace('noise', 'noisy-lj').replace('diffwave','diffwave-lj').replace('pinv', 'pinv-lj')

data['Transcription'] = data['Spreadsheet: pinv'].apply(filename_matcher.find_transcription)
...
# Reorder the columns
data = data[['Participant Private ID', 'audio_type', 'Spreadsheet: pinv', 'Response', 'Transcription']]

# Print the entire dataframe
data.to_excel('./perceptual_results/results_preliminary.xlsx', index=False)
